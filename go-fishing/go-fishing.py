...
def search_zst_beginning(filename):
      fh = open(filename,'rb')
      dctx = zstandard.ZstdDecompressor()
      stream_reader = dctx.stream_reader(fh,read_size=4096)
      text_stream = io.TextIOWrapper(stream_reader, encoding='unicode_escape', newline='\n', errors='ignore')
      first_256_bytes = text_stream.readlines(256)
      for x in first_256_bytes:
          log(log_file, str("DEBUG  "+ x))
          r  = re.search("host::(.{1,50})source::(.{1,50})sourcetype::(.{1,25})\s+",x)
          if r is not None:
             host = str(r.group(1))
...
if __name__ == "__main__":
     log_file="/mnt/data/go-fishing.log"
     q = workqueue(name="fishingqueue", host="10.202.13.180")
     ...
     while not q.empty():
         ...
         if item is not None:
            ...
            search_zst_beginning(journal_file) 
            q.complete(item)
         else:
